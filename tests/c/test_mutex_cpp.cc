...
static opaque_t fail_to_unlock(NativeMutex *m0, NativeMutex *m1) {
...
static opaque_t do_lock(NativeMutex *m0, NativeMutex *m1) {
  ASSERT_TRUE(m0->lock());
  ASSERT_TRUE(m1->lock());
...
TEST(mutex_cpp, simple) {
  ...
  ASSERT_TRUE(succeeder.join(NULL));
...
static opaque_t run_thread(NativeSemaphore *locked, NativeMutex *mutexes,
    size_t *order) {
  // Lock its own mutex.
...
TEST(mutex_cpp, barriers) {
  ...
  // unlocked by it.
  ASSERT_TRUE(threads[kBarrierCount - 1].join(NULL));
  for (size_t i = 0; i < kBarrierCount; i++)
    ASSERT_EQ(i, order[i]);
